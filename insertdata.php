<?php
    ...
    if ($conn->query($q1) === TRUE) {
      ...
      if(isset($_GET['id'])){
        ...
        $typ = $type['type'];
        $mail = $type['email'];
        $sql = "INSERT INTO entrydaily (username,age,name)values('$id','$age','$name')";
        if ($conn->query($sql) === TRUE) {
        } else {
          echo $conn->error;
        }
        $q5 = "INSERT INTO `OUTDATA` (name,email,age,type,username) VALUES('$name','$mail','$age','$typ','$id')";
        if ($conn->query($q5) === TRUE) {
        } else {
          echo $conn->error;
        }
        $sql1="delete from `INDATA` where username = '$id'";
